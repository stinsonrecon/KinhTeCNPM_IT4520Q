...
!(function() {
	 ...
	 * 
	 * jqLite find() is limited to lookups by tag name. TODO This will change with future versions of AngularJS, to be removed when this happens
	 * 
	 * See jqLite.find - why not use querySelectorAll? https://github.com/angular/angular.js/issues/3586 See feat(jqLite): use querySelectorAll instead of
	 * getElementsByTagName in jqLite.find https://github.com/angular/angular.js/pull/3598
	 */
	 ...
	 * 
	 * Taken from AngularUI Bootstrap Typeahead See https://github.com/angular-ui/bootstrap/blob/0.10.0/src/typeahead/typeahead.js#L340
	 */
	.filter(
			'highlight',
			function() {
				...
				return function(matchItem, query) {
					return query && matchItem ? ('' + matchItem).replace(new RegExp(escapeRegexp(query), 'gi'), '<span class="ui-select-highlight">$&</span>')
							: matchItem;
				};
			})
	 ...
	 * 
	 * Taken from AngularUI Bootstrap Position: See https://github.com/angular-ui/bootstrap/blob/master/src/position/position.js#L70
	 */
			...
			function(uiSelectConfig, RepeatParser, uiSelectMinErr, $compile, $window) {
				...
				return {
					...
					compile : function(tElement, tAttrs) {
						...
						choices.attr('ng-repeat', parserResult.repeatExpression(groupByExp)).attr('ng-if', '$select.open'); // Prevent unnecessary watches when
																															// dropdown is closed
						...
						rowsInner.attr('uis-transclude-append', ''); // Adding uisTranscludeAppend directive to row element after choices element has
																		// ngRepeat
	 ...
	 * 
	 * The goal is to limit dependency on the DOM whenever possible and put as much logic in the controller (instead of the link functions) as possible so it
				...
				function _handleDropDownSelection(key) {
					...
					case KEY.ENTER:
						if (ctrl.open && (ctrl.tagging.isActivated || ctrl.activeIndex >= 0)) {
							ctrl.select(ctrl.items[ctrl.activeIndex], ctrl.skipFocusser); // Make sure at least one dropdown item is highlighted before adding
																							// if not in tagging mode
						} else {
							ctrl.activate(false, true); // In case its the search input in 'multiple' mode
						}
						break;
							...
							function onDocumentClick(e) {
								...
								if (!contains && !$select.clickTriggeredSelect) {
									...
									if (!$select.skipFocusser) {
										...
										if (!skipFocusser)
											skipFocusser = ~focusableControls.indexOf(e.target.tagName.toLowerCase()); // Check if target is input, button or
																														// textarea
									} else {
										skipFocusser = true;
									}
							...
							var calculateDropdownPosAfterAnimation = function() {
								...
								$timeout(function() {
									if ($select.dropdownPosition === 'up') {
										...
										var scrollTop = $document[0].documentElement.scrollTop || $document[0].body.scrollTop; // To make it cross browser
																																// (blink, webkit, IE, Firefox).
										...
										scope.$watchCollection(function() {
											...
											if (oldValue != newValue) {
												...
												if (angular.isDefined(ngModel.$modelValue)) {
													ngModel.$modelValue = null; // Force scope model value and ngModel value to be out of sync to re-run
																				// formatters
												}
										...
										$select.searchInput.on('keyup', function(e) {
											...
											if ($select.tagging.isActivated && $select.search.length > 0) {
												...
												if ($select.tagging.fct !== undefined) {
													...
													// remove the first element, if it has the `isTag` prop we generate a new one with each keyup, shaving the
													// previous
	 ...
	 * Debounces functions
	 * 
	 * Taken from UI Bootstrap $$debounce source code See https://github.com/angular-ui/bootstrap/blob/master/src/debounce/debounce.js
	 * 
	 */
	 ...
	 * 
	 * Taken from AngularJS ngRepeat source code See https://github.com/angular/angular.js/blob/v1.2.15/src/ng/directive/ngRepeat.js#L211
	 * 
			...
			.service(
					...
					[
							...
							function(uiSelectMinErr, $parse) {
								...
								/**
								 * Example: expression = "address in addresses | filter: {street: $select.search} track by $index" itemName = "address", source =
								 * "addresses | filter: {street: $select.search}", trackByExp = "$index",
								 */
						...
						function($templateCache) {
							...
							$templateCache
									.put(
											...
											"<span class=\"ui-select-match\"><li class=\"ui-select-match-item select2-search-choice\" ng-repeat=\"$item in $select.selected\" ng-class=\"{\'select2-search-choice-focus\':$selectMultiple.activeMatchIndex === $index, \'select2-locked\':$select.isLocked(this, $index)}\" ui-select-sort=\"$select.selected\"><span uis-transclude-append=\"\"></span> <a href=\"javascript:;\" class=\"ui-select-match-close select2-search-choice-close\" ng-click=\"$selectMultiple.removeChoice($index)\" tabindex=\"-1\"></a></li></span>");
							$templateCache
									.put(
							...
							$templateCache
									.put(
											...
											"<div class=\"ui-select-container selectize-control single\" ng-class=\"{\'open\': $select.open}\"><div class=\"selectize-input\" ng-class=\"{\'focus\': $select.open, \'disabled\': $select.disabled, \'selectize-focus\' : $select.focus}\" ng-click=\"$select.open && !$select.searchEnabled ? $select.toggle($event) : $select.activate()\"><div class=\"ui-select-match\"></div><input type=\"search\" autocomplete=\"off\" tabindex=\"-1\" class=\"ui-select-search ui-select-toggle\" ng-click=\"$select.toggle($event)\" placeholder=\"{{$select.placeholder}}\" ng-model=\"$select.search\" ng-hide=\"!$select.searchEnabled || ($select.selected && !$select.open)\" ng-disabled=\"$select.disabled\" aria-label=\"{{ $select.baseTitle }}\"></div><div class=\"ui-select-choices\"></div></div>");
						} ]);