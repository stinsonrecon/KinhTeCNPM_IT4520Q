...
let exportExcel = (data, columns , fileName) =>{
    overLoading('Loading...');
    try {
        let workbook = excelbuilder.createWorkbook()
        let header = columns.map(item => item.header);
        let sheet1 = workbook.createSheet('sheet1', header.length, data.length + 1);
        for (let iHeader = 1; iHeader <= header.length; iHeader++) {
            sheet1.set(iHeader, 1, header[iHeader - 1]);
            sheet1.border(iHeader, 1, {
                    left: 'thin',
                    top: 'thin',
                    right: 'thin',
                    bottom: 'thin'
            ...
            if (columns[iHeader - 1]?.width !== null && columns[iHeader - 1]?.width !== undefined
                && columns[iHeader - 1]?.width > 0) {
                sheet1.width(iHeader, columns[iHeader - 1]?.width);
            }
            sheet1.align(iHeader, 1, 'center');
            sheet1.valign(iHeader, 1, 'center');
            sheet1.font(iHeader, 1, {bold: 'true'});
            sheet1.fill(iHeader, 1, {type: 'solid', fgColor: 'C7C7C9', bgColor: 'C7C7C9'});
        }
        sheet1.height(1, 20);
        let i = 2;
        for (let x1 of data) {
            let column = columns.map(item => item.key);
            let rowData = [];
            for (let y of column) {
                rowData.push(x1[y] || " ");
            }
            for (let j = 1; j <= rowData.length; j++) {
                sheet1.set(j, i, rowData[j - 1]);
                sheet1.border(j, i, {
                        left: 'thin',
                        top: 'thin',
                        right: 'thin',
                ...
                sheet1.align(j, i, columns[j - 1]?.align === null || columns[j - 1]?.align === undefined || columns[j - 1]?.align === '' ? 'left' : columns[j - 1]?.align);
                sheet1.valign(j, i, columns[j - 1]?.valign === null || columns[j - 1]?.valign === undefined || columns[j - 1]?.valign === '' ? 'center' : columns[j - 1]?.valign);
                sheet1.wrap(j, i, 'true');
                sheet1.numberFormat(j, i, 49); // equivalent to above
            }
            i++;
        }
        workbook.generate(function (err, jszip) {
            if (err) {
                bootboxAlertFocus(err, "", $translate.instant("vnm.lable.vnm.name"), "");
                return;
            }

            jszip.generateAsync({type: "blob", mimeType: 'application/vnd.ms-excel;'}).then(function (blob) {
                let filename = "Result_" + moment(new Date()).format("DDMMYYYYHHmmss")+ (isNullOrEmptyUntil(fileName) ? '.xlsx' : ("_" + fileName));
        ...
        closeOverLay();
        bootboxAlertFocus("Có lỗi xảy ra trong quá trình tải file." +e.message, "", "Vietnamobile", "");
    }
    closeOverLay();
}
